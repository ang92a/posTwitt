...
import './weather.css';
import './weatherFonts.css';
import './weatherFonts2.css';
...
function Weather(): JSX.Element {
  ...
  function success(position: any): void {
    const { longitude, latitude } = position.coords; // достаем координаты

    const API_URL = `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=b582f0a6371524e7a56d1655c5abe090`;
    const API_URL_CITY = `http://api.openweathermap.org/geo/1.0/reverse?lat=${latitude}&lon=${longitude}&appid=b582f0a6371524e7a56d1655c5abe090`;
  ...
  <div class="card_city_container">
  <p class="card_city">in ${obj.city}</p>
  <button class="changeCity">Change city</button>
</div>
<div class="card_temp_container">
  <p class="card_temp">${obj.temp}&#8451;</p>
  <p class="card_icon">${obj.weather}</p>
</div>
...
`;
  ...
  return (
    <div className="wrapper_weathe">
      <div className="card_weather">
        <p className="textUp">Weather</p>
